...
| 屏幕节点标注列表（mark）             | ✅ GET /api/mark         | 否（experimental） |
| 服务端 Overlay 开关与配置           | ✅ GET/POST /api/overlay | 否（experimental） |
| 截图带标注（服务端 overlay annotate） | ✅ GET /api/screenshot?annotate=true | 否（experimental） |
...
> 注：`/api/mark`、`/api/overlay`、`annotate screenshot` 属于 **experimental** 能力，优先用于 AX 提升与调试可观测性，不影响现有 v1/v2 控制路径兼容性。  
> 其中 `/api/overlay` 已支持 `offset_x/offset_y`（用于上屏偏移），并返回 overlay 状态字段 `offsetX/offsetY`；overlay 渲染采用多色语义分组（交互、输入、按钮、选择控件、列表容器、文本节点）以提升可读性。
